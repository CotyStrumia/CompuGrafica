...
shader_program = ShaderProgram(
    window.ctx,
    str(shader_dir / "basic.vert"),
    str(shader_dir / "basic.frag")
)

shader_dir_skybox = Path(__file__).resolve().parents[1]/ "shaders"
shader_program_skybox = ShaderProgram(
    window.ctx,
    str(shader_dir_skybox / "sprite.vert"),
    str(shader_dir_skybox / "sprite.frag")
)
...
if SCENE_TYPE == "normal":
    ...
    scene = RaySceneGPU(window.ctx, WIDTH, HEIGHT, camera, sprite, material_sprite)
    scene.add_object(cube1, material_plastic)
    scene.add_object(cube2, material_glass)
    scene.add_object(quad, material_ceramic)
