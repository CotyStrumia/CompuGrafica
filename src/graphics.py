...
class ComputeGraphics(Graphics):
    ...
    def create_inverse_transformation_matrix(self, inverse_transformations_matrix, index):
        m = self.__model.get_model_matrix()
        inverse_m = glm.inverse(m)
        inverse_transformations_matrix[index, :] = np.array(inverse_m.to_list(), dtype=np.float32).reshape(16)

    def create_materials_matrix(self, materials_matrix, index):
