...
class RayTracerGPU:
    def __init__(self, ctx, camera, width, height, output_graphics):
        ...
        raytrace_comp_path = str(shader_dir_1 / "raytrace.comp")
        self.compute_shader = ComputeShaderProgram(ctx, raytrace_comp_path)
    ...
    def run(self):
        group_x = (self.width + 15) // 16
        group_y = (self.height + 15) // 16

        self.compute_shader.run(group_x=group_x, group_y=group_y, group_z=1)
